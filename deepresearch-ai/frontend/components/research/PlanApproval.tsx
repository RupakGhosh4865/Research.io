  ...
  return (
    <div className="bg-[#1A1F2E] border border-green-500/30 rounded-xl p-6 mb-6">
      ...
      <div className="flex gap-4">
        <button onClick={() => handleApprove(true)} disabled={loading} className="bg-green-500/20 text-green-400 border border-green-500/50 px-6 py-2 rounded-lg font-bold hover:bg-green-500/30 transition-colors">
          Looks Good — Start Research
        </button>
        <button onClick={() => handleApprove(false)} disabled={loading} className="text-red-400 hover:bg-red-500/10 px-6 py-2 rounded-lg transition-colors">
          Cancel
        </button>
      </div>
    </div>
